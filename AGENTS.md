...
- **形态**：host-only 插件（无 client 半），bundle 插件，`cordis.patch.yml` 只有一行 insert。
...
- **构建**：`bash scripts/build.sh`（走 DSH_CHECKOUT 里的 tsc，需要一份 checkout）。
...
框架的 `isAgentLoopRequest` 依据是按对象身份记录的弱集合，构建时链接的
`@deepseek-ai/dsh-llm` 与运行时宿主加载的那份是两个模块实例，弱集合不共享，判断永远是假。
改用 `Object.isFrozen(request)` 加 `purpose === undefined` 判断。
