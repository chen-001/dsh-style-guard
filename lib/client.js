	...
	factory: (require) => {
		...
		*/
		...
		function detail(entry) {
			...
			}, "这一版改写被驳回，页面上用的还是原文"), (0, react.createElement)("div", { key: "b" }, entry.missing.length > 0 ? "它丢掉了原文里的 " + entry.missing.join("、") + "。下面把它整段列出来，原文里对应的词标了红色，改写版里底色发黄的句子在原文里找不到对应，看的时候要对着原文核一遍。" : "下面把它整段列出来，底色发黄的句子在原文里找不到对应，看的时候要对着原文核一遍。")]));
			...
			}, [textBlock(entry.missing.length > 0 ? "原版（红色是改写版丢掉的词）" : "原版", entry.missing.length > 0 ? withHighlights(entry.original, entry.missing) : plain(entry.original), "#fafafa"), textBlock(rejectedOnly ? "改写后（被驳回，黄底句子请对照原文）" : shown ? "改写后" : "改写后（无，未产生可用的改写）", rejectedOnly ? withRiskyMarks(shown, riskySentences(entry.original, shown)) : plain(shown), rejectedOnly ? "#fffdf5" : "#f4f8ff")]));
